...
def items_post():
	"""
	Creates a new items. Allowed only for warehouseman and admin roles.
	---
	parameters:
		- in: header
		  name: Authorization
...
def item_book():
	...
	for i in items:
		...
		if current_user().user_money < 0:
			raise MoneyError()
		item.book(current_user_id(), count)
...
def item_unbook():
	...
	responses:
	  ...
	  403:
	    description: "Given wrong use_id or wrong quantity, or You are trying to access items that
...
def approve_item():
	...
	for i in useIds:
		...
		item.item_quantity_current -= itemRecord.item_quantity
...
"""
половину всей хуйни по-хорошему в models хранить, заебало короче
"""
@bp.route('/items/use/take', methods=['POST'])
@roles_accepted('warehouseman', 'admin')
def take_item():
	req = request.get_json(force=True)
	useIds = req.get('useIds', None)
	...
	for i in useIds:
		...
		itemRecord.until_datetime = datetime.utcnow() + timedelta(days=60)
	db.session.commit()
	return Response(status=200)
...
def book_reject():
	req = request.get_json(force=True)
	useIds = req.get('useIds', None)
	quantities = req.get('quantities', None)
	if useIds is None or quantities is None:
		raise JSONNotEnoughError()
	for i in range(len(useIds)):
		itemRecord = ItemInUse.search_by_use_id(useIds[i]['use_id'])
		if itemRecord is None:
			raise ElementNotFoundError()
		itemRecord.unbook(quantities[i])
	db.session.commit()
	return Response(status=200)
